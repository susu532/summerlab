...
export class InteractionSystem implements ISystem {
  ...
  updateRaycast(force = false) {
    ...
    if (force || now - this._lastRaycastTime > 66) {
      // roughly 15fps
      this._lastRaycastTime = now;
      this._tempRaycastDir
        .set(
          -Math.sin(this.game.player.cameraYaw) *
            Math.cos(this.game.player.cameraPitch),
          Math.sin(this.game.player.cameraPitch),
          -Math.cos(this.game.player.cameraYaw) *
            Math.cos(this.game.player.cameraPitch),
        )
        .normalize();
      const ray = this.game.world.raycast(
        this.game.player.playerHeadPos,
        this._tempRaycastDir,
        5,
      );
      const npcRay = this.game.entityManager.raycastNPC(
        this.game.player.playerHeadPos,
        this._tempRaycastDir,
        5,
        this.game.camera,
  ...
  update(delta: number) {
    ...
    if (ray.hit && this.selectionBox) {
      ...
      const pulse = isPerformanceMode
        ? 1.0
        : 1.0 + Math.sin(this.game.clock.getElapsedTime() * 10) * 0.01;
    ...
    if (
      ...
      if (
        ...
        if (this.game.chocolateFluidSystem) {
          ...
          let camDir = new THREE.Vector3(0, 0, -1).transformDirection(
          ...
          if (this.game.player.perspective === Perspective.FIRST_PERSON) {
            ...
            if (
              this.game.player.renderer.fpHeldItemModel &&
              this.game.player.renderer.fpHeldItemModel.visible
            ) {
              const nozzle =
                this.game.player.renderer.fpHeldItemModel.getObjectByName(
                  "hose_nozzle",
                );
              ...
              const targetPoint = this.game.camera.position
                .clone()
                .add(camDir.clone().multiplyScalar(20));
            ...
            origin = new THREE.Vector3();
            if (
              this.game.player.renderer.heldItemModel &&
              this.game.player.renderer.heldItemModel.visible
            ) {
              const nozzle =
                this.game.player.renderer.heldItemModel.getObjectByName(
                  "hose_nozzle",
                );
              ...
              origin = this.game.player.worldPosition
                .clone()
                .add(new THREE.Vector3(0, 1.1, 0));
              origin.add(camDir.clone().multiplyScalar(0.7));
              const right = new THREE.Vector3(1, 0, 0).applyQuaternion(
                this.game.player.modelGroup.quaternion,
              );
          ...
          const fluidColor =
            equippedItem.type === ItemType.WASHING_HOSE
              ? new THREE.Color("#3889f0")
              : new THREE.Color(
                  useGameStore.getState().fluidColor || "#3d1c04",
                );
          this.game.chocolateFluidSystem.emit(
            origin,
            emitDir,
            isSpray,
            fluidColor,
