...
export class PlayerRenderer {
  ...
  public update(delta: number, isGliding: boolean) {
    ...
    if (this.player.isSpectator) {
      ...
      this.fpArmGroup.visible =
        this.isHandVisible && this.player.perspective === 0;

      const isChargingBow =
        this.player.inventory.slots[this.player.hotbarIndex]?.type ===
          ItemType.BOW && this.player.inputController.bowChargeStart > 0;
      this.fpOffHandArmGroup.visible =
        this.isHandVisible && this.player.perspective === 0 && !isChargingBow;

      // Also hide torch light and 3rd person off-hand if charging bow
      if (this.torchLight) {
        const hasTorch =
          this.player.inventory.slots[36]?.type === ItemType.TORCH ||
          this.player.inventory.slots[this.player.hotbarIndex]?.type ===
            ItemType.TORCH;
  ...
  private updateItemHand(type: number, isOffHand: boolean) {
    ...
    const fpModelGrp = isOffHand
      ? this.fpOffHandHeldItemModel
      : this.fpHeldItemModel;
    const currentModelType = isOffHand
      ? this.currentOffHandModelType
      : this.currentModelType;
    const currentFpModelType = isOffHand
      ? this.currentFpOffHandModelType
  ...
  animate(delta: number) {
    ...
    if (isMoving && this.player.canJump && !this.player.isFlying) {
      ...
      if (Math.sin(this.player.walkCycle) < 0 && Math.sin(oldWalkCycle) >= 0) {
        const blockBelow = this.player.world.getBlock(
          ...
          Math.floor(
            this.player.worldPosition.y - this.player.playerHeight - 0.1,
          ),
      ...
      this.player.walkCycle = THREE.MathUtils.lerp(
    ...
    const verticalMovement =
      ...
      delta;
    const isClimbing =
      isMoving && verticalMovement > 0.5 && this.player.canJump;
    this.player.climbTransition = THREE.MathUtils.lerp(
      this.player.climbTransition,
      isClimbing ? 1 : 0,
      delta * 10,
    );
    if (!this.player.lastWorldPosition)
      this.player.lastWorldPosition = new THREE.Vector3();
    ...
    if (
      ...
      if (this.player.isFlying) {
        ...
        const kickAngle =
          Math.cos(this.player.walkCycle * swimSpeed * 1.5) * 0.4;
        ...
        if (this.player.climbTransition > 0.01) {
          ...
          const stepLift =
            Math.max(0, Math.sin(this.player.walkCycle)) * 0.5 * t;
          const stepLiftAlt =
      ...
      if (this.player.crouchTransition > 0.01) {
        ...
        if (this.player.neckMesh)
          this.player.neckMesh.scale.y = 1.0 / bodyScaleY;
      ...
      if (
        ...
        if (this.heldItemType === ItemType.BOW) {
          ...
          if (this.player.isSwinging) {
            this.player.rightArmMesh.rotation.x +=
              Math.sin(this.player.swingTimer) * 0.5 * t;
          }
        }
      } else {
      ...
      if (this.player.isSwinging && this.player.blockTransition < 0.5) {
        ...
        const equippedItem =
          this.player.inventory.slots[this.player.hotbarIndex];
        const isSword =
          equippedItem &&
      ...
      const headPitchOffset =
        this.player.crouchTransition > 0.01
          ? -(this.player.crouchTransition * 0.4)
          : 0;
