...
export function updatePlayer(player: Player, delta: number) {
  ...
  for (const item of player.entityManager.droppedItems.values()) {
    ...
    if (player.world.isSummerLab) {
      const isSpecialHoseItem =
        item.type === ItemType.FLUID_CHOCOLATE_HOSE ||
      ...
      if (isSpecialHoseItem) {
        ...
        const hasAnySpecialItem =
          checkInventoryCount(ItemType.FLUID_CHOCOLATE_HOSE) > 0 ||
  ...
  if (
    equipStack?.type === ItemType.BOW &&
    player.inputController.bowChargeStart > 0
  ) {
    ...
    if (
      ...
      if (use3DModel) {
        ...
        if (itemTypeNum === ItemType.BOW) {
          ...
          const stringMesh =
            player.fpHeldItemModel.getObjectByName("bow_string");

          if (player.inputController.bowChargeStart > 0) {
            const chargeTime =
              Date.now() - player.inputController.bowChargeStart;
  ...
  if (player.perspective !== Perspective.FIRST_PERSON) {
    ...
    const isChargingBow =
      player.inventory.slots[player.hotbarIndex]?.type === ItemType.BOW &&
      player.inputController.bowChargeStart > 0;
    player.fpOffHandArmGroup.visible =
      player.renderer.isHandVisible && !isChargingBow;
  ...
  const currentState = {
    ...
    isBlocking:
      player.isBlocking ||
      (player.inputController.isRightMouseDown &&
        (player.inventory.slots[player.hotbarIndex]?.type ===
          ItemType.FLUID_CHOCOLATE_HOSE ||
          player.inventory.slots[player.hotbarIndex]?.type ===
            ItemType.WASHING_HOSE)),
    isShooting:
      (player.inputController.isRightMouseDown || player.isLeftMouseDown) &&
      (player.inventory.slots[player.hotbarIndex]?.type ===
        ItemType.FLUID_CHOCOLATE_HOSE ||
        player.inventory.slots[player.hotbarIndex]?.type ===
          ItemType.WASHING_HOSE),
    fluidColor:
      player.inventory.slots[player.hotbarIndex]?.type === ItemType.WASHING_HOSE
        ? 0x3889f0
        : parseInt(useGameStore.getState().fluidColor.replace("#", "0x")) || 0,
