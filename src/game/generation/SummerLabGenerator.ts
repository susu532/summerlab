...
export function getWaterParkBlock(x: number, y: number, z: number): number {
  ...
  if (fy <= 0 && fy >= -5) {
      if (fy === 0) {
          if (Math.abs(x) <= 4 || Math.abs(z) <= 4) {
              baseBlock = ItemType.CONCRETE_WHITE; // Main walkways
          ...
          if (dist >= 45 && dist <= 52) {
              if (!isBridge) {
                  ...
                  baseBlock = ItemType.PLANKS; // Bridge
              }
          } else if (dist >= 44 && dist <= 53) {
              if (dist < 45 || dist > 52) baseBlock = ItemType.CONCRETE_WHITE; // Borders
          }
          ...
          if (isPool(25, 25, 14, 10)) {
              ...
              else baseBlock = ItemType.CONCRETE_WHITE;
          }
          // Q2 Diving/Deep Pool
          ...
          if (checkPoolWater(25, 25, 14, 10)) { // Leisure Pool
              ...
              else baseBlock = ItemType.CONCRETE_RAINBOW_BLUE; // Deep blue bottom
  ...
  if (baseBlock === ItemType.AIR && fy > 0) {
      ...
      const isCabana = (cx: number, cz: number) => Math.abs(x - cx) <= 2 && Math.abs(z - cz) <= 2;
      const cabanas = [[15, 38], [25, 38], [35, 38], [38, 25], [38, 15]];
      ...
      // 2. Lifeguard Stands
      const stands = [[10, 25], [-10, 25], [25, -10], [-10, -25]];
      for (const [sx, sz] of stands) {
          if (Math.abs(x - sx) <= 1 && Math.abs(z - sz) <= 1 && fy <= 5) {
              if (fy <= 3) {
                  ...
                  if (x === sx && z === sz) baseBlock = ItemType.CONCRETE_RAINBOW_RED; // seat
                  else if (Math.abs(x - sx) === 1 || Math.abs(z - sz) === 1) baseBlock = ItemType.PLANKS; // railing
              } else if (fy === 5 && x === sx && z === sz) {
                  baseBlock = ItemType.CONCRETE_RAINBOW_RED; // Top umbrella
              }
          }
      }
      ...
      if (dist >= 53 && dist <= 58) {
          ...
          if (x >= 53 && x <= 56 && z >= -10 && z <= 10) {
               if (fy <= 4) {
                   ...
                   else if (x === 53 && fy === 1) baseBlock = ItemType.CONCRETE_WHITE; // counter base
                   ...
                   else if (x > 53) baseBlock = ItemType.CONCRETE_WHITE; // back kitchen walls
               }
          }
      }
      ...
      if (Math.abs(x) <= 8 && z >= 53 && z <= 58 && fy <= 5) {
           if (x >= -6 && x <= -2 || x >= 2 && x <= 6) {
               if (fy === 5) baseBlock = ItemType.CONCRETE_RAINBOW_INDIGO;
               else if (fy === 2 && z === 53 && Math.abs(x) !== 4) baseBlock = ItemType.GLASS; // windows
               else baseBlock = ItemType.CONCRETE_WHITE;
           }
      }

      // 5. Diving Boards (Q2: -25, 25)
      if (x >= -24 && x <= -16 && z >= 22 && z <= 26 && fy <= 10) {
          // 3m board
          if (x === -20 && z === 25 && fy <= 3) baseBlock = ItemType.WOOD;
          if (x >= -24 && x <= -20 && z === 25 && fy === 3) baseBlock = ItemType.SLAB_WOOD;
          // 5m board
          if (x === -20 && z === 22 && fy <= 5) baseBlock = ItemType.WOOD;
          if (x >= -24 && x <= -20 && z === 22 && fy === 5) baseBlock = ItemType.SLAB_WOOD;
          // 10m platform tower
          if (x >= -18 && x <= -16 && z >= 23 && z <= 26) {
              if (fy <= 10) {
                  if (Math.abs(x) === 16 || Math.abs(x) === 18) baseBlock = ItemType.STONE; // legs
                  if (fy === 10) baseBlock = ItemType.PLANKS; // platform
                  if (fy < 10 && x === -17 && z === 26) baseBlock = ItemType.WOOD; // ladder
              }
          }
          if (x >= -24 && x <= -18 && z >= 24 && z <= 25 && fy === 10) baseBlock = ItemType.SLAB_STONE; // jumping off 10m
      }
      ...
      if (Math.abs(x) <= 8 && Math.abs(z) <= 8) {
          if (fy <= 30) {
              // Structural pillars
              if (Math.abs(x) === 8 && Math.abs(z) === 8) baseBlock = ItemType.STONE;
              // Glass walls
                  ...
                  if (fy % 5 !== 0) {
                      ...
                      baseBlock = ItemType.STONE; // bands
                  }
              }
              // Intermediary floors
              else if (fy % 10 === 0) {
              ...
              // Spiral staircase
                  ...
                  if (Math.abs(normSAngle - heightMod) < 0.2) {
                      ...
                      baseBlock = ItemType.STONE; // central core
                  }
              }
          } else if (fy <= 38) {
              ...
              baseBlock = ItemType.STONE; // antenna
          }
      }
      ...
      if (fy <= 30 && distToS1Sq <= 12) {
          if (distToS1Sq >= 6) {
              // Tube coloring
              if (fy % 4 === 0) baseBlock = ItemType.CONCRETE_WHITE; // Rings
      ...
      if (x > 8 && x < 25 && z < -8 && z > -25) {
          ...
          if (Math.abs(progX - progZ) <= 3) {
              ...
              if (Math.abs(expectedY - fy) <= 1) {
                  // Two distinct lanes
                  if (Math.abs(progX - progZ) === 1) baseBlock = ItemType.CONCRETE_WHITE; // divider
                  else if (Math.abs(progX - progZ) === 3) baseBlock = ItemType.CONCRETE_RAINBOW_BLUE; // outer walls
                  else {
                      if (expectedY - fy > 0) baseBlock = ItemType.WATER;
                      else baseBlock = ItemType.CONCRETE_RAINBOW_RED;
                  }
              }
              // Support pillars
      ...
      if (x < -8 && x > -25 && z < -8 && z > -25) {
          ...
          if (Math.abs(progX - progZ) <= 4) {
              ...
              if (Math.abs(waveY - fy) <= 1) {
                  ...
                  else baseBlock = ItemType.CONCRETE_RAINBOW_GREEN;
              } else if (Math.abs(waveY - fy) <= 2 && Math.abs(progX - progZ) === 4) {
                  baseBlock = ItemType.CONCRETE_RAINBOW_YELLOW;
              }
              if (Math.abs(progX - progZ) === 0 && progX % 5 === 0 && fy < waveY - 2 && baseBlock === ItemType.AIR) {
                  baseBlock = ItemType.CONCRETE_WHITE;
              }
          }
      }
          ...
          if (Math.abs(x + 25) <= 4 && Math.abs(z + 25) <= 4) {
             if (fy <= 4) {
                if (Math.abs(x + 25) === 4 || Math.abs(z + 25) === 4) {
                   ...
                   else baseBlock = ItemType.AIR; // safety net holes
                 ...
                 if (Math.abs(x + 25) <= 2 && Math.abs(z + 25) <= 2) {
                     if (fy === 10) baseBlock = ItemType.CONCRETE_RAINBOW_RED; // bottom of bucket
                     else if (Math.abs(x + 25) === 2 || Math.abs(z + 25) === 2) baseBlock = ItemType.CONCRETE_RAINBOW_ORANGE; // sides
      ...
      if (fy === 0 && dist < 45 && dist > 10) {
          ...
          const isWaterZone = (x > 12 && z > 12) || (x < -12 && z > 12) || (x < -12 && z < -12) || (x > 12 && z < -12);
          if (isWaterZone) {
