...
export interface StatsSlice {
  ...
  leaderboard: Record<
    string,
    { id: string; name: string; team?: string; kills: number; deaths: number }
  >;
...
export type GameState = InventorySlice &
  ChatSlice &
  PopupSlice &
  CoreGameSlice &
  StatsSlice &
  EnvironmentSlice &
  KillCelebrationSlice &
  SocialSlice;

let messageIdCounter = 0;

const createKillCelebrationSlice: StateCreator<
  GameState,
  [],
  [],
  KillCelebrationSlice
> = (set) => ({
  ...
  addKillCelebration: (victimName, isPlayer, isBot, coinsRewarded) => {
    ...
    set((state) => ({
      killCelebrations: [
        ...state.killCelebrations,
        { id, victimName, isPlayer, isBot, coinsRewarded },
      ],
    }));
    setTimeout(() => {
      set((state) => ({
  ...
  removeKillCelebration: (id) =>
    set((state) => ({
      killCelebrations: state.killCelebrations.filter((c) => c.id !== id),
    })),
});

const createInventorySlice: StateCreator<GameState, [], [], InventorySlice> = (
  set,
) => ({
  inventoryVersion: 0,
...
const createChatSlice: StateCreator<GameState, [], [], ChatSlice> = (set) => ({
  ...
  addChatMessage: (sender, message, team) => {
    ...
    // Detect death messages from System
    const isDeathMessage = sender === "System" && (
      message.includes("was slain") ||
      message.includes("was shot") ||
      message.includes("burned") ||
      message.includes("fell") ||
      message.includes("died") ||
      message.includes("was killed") ||
      message.includes("blew up") ||
      message.includes("hit the ground")
    );
    ...
    if (isDeathMessage) {
      set((state) => {
        setTimeout(() => {
          set((s) => ({
            killFeedMessages: s.killFeedMessages.filter((m) => m.id !== id)
          }));
        }, 8000);
        return {
          killFeedMessages: [
            ...state.killFeedMessages.slice(-5),
            { id, message: message.replace(/§[0-9a-f]/g, "") }, // Remove color codes for pure text
          ]
        };
      });
      return;
    ...
    set((state) => ({
      chatMessages: [
        ...
        { id, sender, message, team },
      ],
    }));
  },
  clearChatMessages: () => set({ chatMessages: [], killFeedMessages: [] }),
});

const createPopupSlice: StateCreator<GameState, [], [], PopupSlice> = (
  set,
) => ({
  xpPopups: [],
  ...
  addLevelUpPopup: (skill, level) => {
    ...
    setTimeout(() => {
      set((state) => ({
        levelUpPopups: state.levelUpPopups.filter((p) => p.id !== id),
      }));
    }, 5000);
  },
  removeLevelUpPopup: (id) =>
    set((state) => ({
      levelUpPopups: state.levelUpPopups.filter((p) => p.id !== id),
    })),
});

const createCoreGameSlice: StateCreator<GameState, [], [], CoreGameSlice> = (
  set,
) => ({
...
const createStatsSlice: StateCreator<GameState, [], [], StatsSlice> = (
  set,
  get,
) => ({
  skycoins: {},
  ...
  setSkycoins: (amount) =>
    set((state) => ({
      skycoins: { ...state.skycoins, [state.currentMode]: amount },
    })),
  addSkycoins: (amount) =>
    set((state) => {
      const current = state.skycoins[state.currentMode] ?? 500;
      return {
        skycoins: { ...state.skycoins, [state.currentMode]: current + amount },
      };
    }),
  playerStats: null,
  ...
  setLeaderboardPlayer: (id, name, team, kills, deaths) =>
    set((state) => ({
      leaderboard: {
        ...state.leaderboard,
  ...
  updateLeaderboardStats: (id, kills, deaths) =>
    set((state) => {
      const p = state.leaderboard[id];
      if (!p) return state;
      return {
        leaderboard: { ...state.leaderboard, [id]: { ...p, kills, deaths } },
      };
    }),
  removeLeaderboardPlayer: (id) =>
    set((state) => {
...
const createSocialSlice: StateCreator<GameState, [], [], SocialSlice> = (
  set,
) => ({
  friendRequests: [],
  partyInvites: [],
  addFriendRequest: (sourceId, sourceName) =>
    set((state) => {
      if (state.friendRequests.find((r) => r.sourceId === sourceId))
        return state;
      return {
        friendRequests: [...state.friendRequests, { sourceId, sourceName }],
      };
    }),
  removeFriendRequest: (sourceId) =>
    set((state) => ({
      friendRequests: state.friendRequests.filter(
        (r) => r.sourceId !== sourceId,
      ),
    })),
  addPartyInvite: (sourceId, sourceName, server) =>
    set((state) => {
      if (state.partyInvites.find((r) => r.sourceId === sourceId)) return state;
      return {
        partyInvites: [...state.partyInvites, { sourceId, sourceName, server }],
      };
    }),
  removePartyInvite: (sourceId) =>
    set((state) => ({
